...
class Edit
{
    ...
    {
        if (isset($data['increment']) && $this->checkIncrementId($data['increment'], $order->getIncrementId())) {
            return [
                'error' => true,
        ...
        if (isset($data['date'])) {
            $order->setCreatedAt($this->timezone->convertConfigTimeToUtc($data['date']));
        }
    ...
    public function setCustomerData($order, $data)
    {
        ...
        $order->setCustomerGender(array_key_exists('gender', $data) ? $data['gender'] : '');

        if (isset($data['modify-type']) && $data['modify-type'] === 'change') {
            ...
            $order->getBillingAddress()->setCustomerAddressId(array_pop($applicableAddressIds));
        }

        /** create/update data for customer info */
        if (isset($data['save-customer'])) {
            if ($customerId) {
                $customer = $this->_customerRepoInterface->getById($customerId);
                $this->setData($customer, $data, $email);
                $this->_customerRepoInterface->save($customer);
            } elseif ($this->checkExistCustomer($email)) {
                try {
                    $customer = $this->_customerRepoInterface->get($email);
                    $this->setData($customer, $data, $email);
    ...
    public function setData($customer, $data, $email)
    {
        ...
        $customer->setGender($data['gender'] ?? 0);
