...
class CustomDiscount
{
     ...
     * @return $this|mixed
     */
    public function afterProcess(Validator $subject, $result, $item)
    {
        if (!$this->_helperData->isEnabled($item->getStore()->getId())) {
            return $result;
        }
            ...
            foreach ($request['item'] as $itemId => $data) {
                if ($itemId === (int)$item->getId()) {
                    if (isset($data['use_custom_discount'])) {
                        ...
                        $value = (is_numeric($data['discount_value']) && $data['discount_value'] > 0) ? $data['discount_value'] : 0;
                        ...
                        $item->setBaseDiscountAmount($discountAmount / $item->getStore()->getCurrentCurrencyRate());
