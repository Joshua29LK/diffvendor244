...
class View extends Template
{
    ...
    public function getFormattedAddress($data)
    {
        ...
        //fix field date Osc
        if ($data['mposc_field_3']) {
            $data['mposc_field_3'] = date("Y-m-d", strtotime($data['mposc_field_3']));
        }
     ...
     * @return mixed
     */
    public function getNewData($key)
    {
        $newData = $this->getNewOrderData();
        if (!isset($newData['order'][$key]['gender'])) {
            $newData['order'][$key]['gender'] = '';
        }
        return $newData['order'][$key];
    }
     ...
     */
    public function getProductBySku($proSku)
    {
        try {
            return $this->productRepository->get($proSku);
        } catch (NoSuchEntityException $e) {
            try {
                $itemCollectionFactory = $this->itemCollectionFactory->create();
                $itemCollectionFactory->addFieldToFilter('order_id', $this->getLog()->getOrderId());
                $itemCollectionFactory->addFieldToFilter('sku', $proSku);
                foreach ($itemCollectionFactory as $item) {
